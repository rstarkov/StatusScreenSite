...
class Entry {
    ...
    private $tdChartHourly: Util.Html;
    ...
    private _plotHourly: Plottable.Plot;
    private _dataHourly: Datasets;
    ...
    public Add($tbody: Util.Html): void {
        this.$row = $(`
            <tr>
                ...
                <td id=hpChartHourly class="plot hourly"><div class="plot hourly"></div></td>
        ...
        this.$tdChartHourly = Util.$get(this.$row, 'td#hpChartHourly');
    ...
    public Update(dto: any): void {
        ...
        if (this._dataHourly) {
            this._dataHourly.prc01.data(dto.Hourly);
            this._dataHourly.prc50.data(dto.Hourly);
            this._dataHourly.prc75.data(dto.Hourly);
            this._dataHourly.prc95.data(dto.Hourly);
        }
        if (this._dataDaily) {
    ...
    private initialisePlots(dto: any): void {
        ...
        let yScale: Plottable.Scale<{}, number>;
        if (false)
            yScale = new Plottable.Scales.Linear().domainMin(0).domainMax(1000);
        else
            yScale = new Plottable.Scales.ModifiedLog(10).domainMin(dto.Last30d.MsResponsePrc01 * 0.9).domainMax(dto.Last30d.MsResponsePrc50 * 3)
        ...
        // Two min, hourly & daily chart
        if (false) {
            yScale = new Plottable.Scales.Linear().domainMin(0).domainMax(101);
            yScale = new Plottable.Scales.ModifiedLog(10).domainMin(0).domainMax(101);
            [this._data2min, this._plot2min] = this.initUptimePlot(Util.get(this.$tdChart2min, 'div.plot'), yScale);
            [this._dataHourly, this._plotHourly] = this.initUptimePlot(Util.get(this.$tdChartHourly, 'div.plot'), yScale);
            [this._dataDaily, this._plotDaily] = this.initUptimePlot(Util.get(this.$tdChartDaily, 'div.plot'), yScale);
        }
        else {
            [this._data2min, this._plot2min] = this.initStackedPlot(Util.get(this.$tdChart2min, 'div.plot'), yScale);
            [this._dataHourly, this._plotHourly] = this.initStackedPlot(Util.get(this.$tdChartHourly, 'div.plot'), yScale);
            [this._dataDaily, this._plotDaily] = this.initStackedPlot(Util.get(this.$tdChartDaily, 'div.plot'), yScale);
        }
    }

    private initUptimePlot(target: HTMLElement, yScale: Plottable.Scale<{}, number>): [Datasets, Plottable.Plot] {
        var datas = {
            prc01: new Plottable.Dataset([], { type: null, color: "green" }),
            prc50: new Plottable.Dataset([], { type: "ErrorCount", color: "yellow" }),
            prc75: new Plottable.Dataset([], { type: "TimeoutCount", color: "red" }),
            prc95: new Plottable.Dataset([], { type: "DUMMY", color: "blue" }),
            // FIXME!
        };
        let xScale = new Plottable.Scales.Linear().domainMin(-1).domainMax(30);
        let colorScale = new Plottable.Scales.Color()
        ...
        var plot = new Plottable.Plots.StackedBar()
            ...
            .y((pt, i, ds) => { return pt.TotalCount == 0 ? (ds.metadata().type ? 0 : 100) : ((ds.metadata().type ? pt[ds.metadata().type] : (pt.TotalCount - pt.ErrorCount - pt.TimeoutCount)) / pt.TotalCount * 100); }, yScale)
            .attr('fill', (pt, i, ds) => { return pt.TotalCount == 0 ? "grey" : ds.metadata().color; }, colorScale)
